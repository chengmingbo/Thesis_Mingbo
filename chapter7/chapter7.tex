...
The boundary matrix on 1-simplices $\mathbf{B}_1$ is defined as in \eref{eqn:incidencematrix1}. An entry in $\mathbf{B}_2$ capturing the relationship between an directed edge $e_i \in \mathcal{E}$ and a directed triangle $\bigtriangleup_{q} \in \mathcal{T}$ can be defined as:
\begin{equation}
...
\begin{figure}[!ht]
    ...
    \vspace{0.1cm}
    \caption[Number of eigens selecting algorithm.]{\textbf{Number of eigens selecting algorithm.} The eigenvalues are derived from the benchmarking data, specifically from the Diffusion Limited Aggregation Tree with 10 branches~\trefp{tab:DLA_trees}.}
...
\begin{algorithm}
    ...
    \Parameter{
        ...
        $\theta$, Percentage of starting cells\\
    }
    \Output{
    ...
    $\mathcal{V}_{low}^{knn} \gets \textbf{LowestPseudotime}(\mathcal{V}^{t}, \theta)$ \\
...
\begin{algorithm}
    ...
    \For{$i\gets1$ \KwTo $m$ \KwBy $1$}{
        ...
        $\text{BINs}(i) \gets (t_i^{1}, t_i^{2}, \cdots t_i^{n_{bin}})$\\
        $V_i \gets (V_i^{1}, V_i^{2}, \cdots V_i^{n_{bin}})$ \Comment*[r]{split the $V_{i}$ to be $n_{bin}$}
    }
    $\text{DIC} \gets \{\}$\Comment*[r]{dictionary stores the branching points of all pairs}
    \For{$i\gets1$ \KwTo $m$ \KwBy $1$}{
        %
        \For{$j\gets i+1$ \KwTo $m$ \KwBy $1$ $~\& j \neq i$}{
            $\text{Len}\gets \textbf{min}(\textbf{length}(V_i), \textbf{length}(V_j))$\\
    ...
    $\text{bps} \gets \textbf{order}(\textbf{values}(\text{DIC}), \text{ascend}=\text{False})$ \Comment*[r]{ordered branching points ids}
