...
%3-12
\begin{algorithm}
\caption{DLA Tree Generation}
\label{alg:DLA}
\Input{$n\_samp, n\_dim, n\_branch, \sigma$}
    \textbf{seed random number generator}\\
    $n\_steps \gets \lfloor\frac{n\_samp}{n\_branch}\rfloor$\\
    $M \gets \text{cumsum}(-1 + 2 \times \text{random\_numbers}(n\_steps, n\_dim)$\\

    \While{$I \gets 1$ to $n\_branch$}
    {
         $ind \gets \text{random\_choice}(\text{range}(M.\text{rows}))$\\
         $M2 \gets \text{cumsum}(-1 + 2 \times \text{random\_numbers}(n\_steps, n\_dim)$\\
        $M \gets \text{concatenate}(M, \text{tile}(M[ind, :], (n\_steps, 1)) + M2)$\\

     $C \gets \text{tile}(\text{range}(1, n\_branch + 1), (n\_steps, 1)).\text{transpose}().\text{flatten}()$\\
     $M \gets M + \text{random\_normal}(0, \sigma, \text{size}=M.\text{shape})$\\
    }
    \Return \{$M[:n\_samp]$, $C[:n\_samp]$\}\\
\end{algorithm}
