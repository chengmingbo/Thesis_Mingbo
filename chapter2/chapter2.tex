...
\begin{figure}[!ht]
	...
	\vspace{0.1cm}
	\caption[central dogma.]{The figure illustrates DNA transcription and RNA translation central dogma.} 
...
Various techniques capable of simultaneously measuring the transcriptome and proteome within a single cell have been created. These methods include proximity extension assay/specific RNA target amplification (PEA/STA), proximity ligation assay for RNA (PLAYR), cellular indexing of transcriptomes and epitopes by sequencing(CITE-seq)\citep{stoeckius2017citeseq} and RNA expression and protein sequencing assay (REAP-seq)\citep{peterson2017reapseq}. And, expanded CRISPR-compatible cellular indexing of transcriptomes and epitopes by sequencing(ECCITE-seq)\citep{mimitou2019ECCITE} is modified and extended from CITE-seq. ECCITE-seq offers diverse modalities of information, including transcriptome, protein, clonotype, and CRISPR perturbation data at a single-cell resolution. Take CITE-seq as an example, it uses DNA-barcoded antibodies to convert the detection of proteins into a quantitative, sequenceable readout. Oligos bound to antibodies serve as synthetic transcripts, captured in the majority of large-scale oligo(dT)-based single-cell RNA sequencing (scRNA-seq) library preparation protocols like 10x Genomics, Drop-seq


 Antibody-bound oligos act as synthetic transcripts that are captured during most large-scale oligodT-based scRNA-seq library preparation protocols (e.g. 10x Genomics, Drop-seq

Several Methods for single-cell multimodal analyses of the genome and transcriptome have been developed, including gDNA-mRNA sequencing (DR-seq)\citep{dey2015drseq}, genome and transcriptome sequencing(G\&T-seq)\citep{macaulay2015gttseq}, simultaneous isolation of genomic DNA and total RNA (SIDR)\citep{han2018sidr} and TARGET-seq\citep{rodriguez2019targetseq} and single cell triple omics sequencing (scTrio-seq)\citep{hou2016sctrioseq}. 

To date, several methods have been developed for sequencing proteomes, transcriptomes and epigenomes simultaneously at single cell level. TEA-seq\citep{swanson2021simultaneous} is designed as a trimodal assay that simultaneously measures transcriptomics (scRNA-seq), epitopes, and chromatin accessibility (scATAC-seq) from thousands of single cells while DOGMA-seq\citep{mimitou2021scalable} can measure all features of TEA-seq and also the mtDNA mutation at the same time.
...
\begin{description}
	\item[dimensional reduction] 
	Unlike the scRNA data, usually, scATAC uses Latent semantic indexing(LSI) to reduce the dimensions, which were first introduced for the analysis of scATAC-seq data by ~\citep{cusanovich2015multiplex}. It combines the steps of TF-IDF followed by Singular value decomposition(SVD). Signac~\citep{signac} use LSI as the default dimension reduction methods, whereas ArchR implemented Iterative Latent Semantic Indexing(iterativeLSI)~\citep{satpathy2019massively, granja2019single} which initiates by performing an initial Latent Semantic Indexing (LSI) transformation on the most accessible bins and identified lower-resolution clusters that remain unaffected by batch confounding. To note, usually the first LSI component captures sequencing depth, it's recommended to check the correlation between each component and the sequencing depth to exclude the noise introduced for the downstream analysis~\citep{signac}. For visualization, UMAP and t-SNE are the best choices in this scenario.
...
The surface protein analysis is simpler than scRNA and scATAC-seq, It's often profiled with scRNA-seq. Therefore, the quality check is usually done by detecting single-cell RNA feature distributions. We can perform similar normalization, and dimensional reduction as scRNA-seq. 
...
Single-cell multi-modal profiling is a new area. And the task of the integration of single-cell multimodal, although there are some tools have been developed to achieve the task of integration, there are some tools have  

\begin{itemize}
	\item \textbf{Heterogeneous statistical properties:}
	 Different modalities hold distinct statistical properties.  For example, the feature size of protein usually is around 100. ATAC feature space size is hundreds of thousands, And Gene expression matrix feature space size is 10s of 1000. The sparsity of different modalities is diverse, protein counts are usually dense and ATAC peaks are sparse.
	feature size :protein(~100), ATAC(~100,000),  RNA(~10,000)
	sparsity levels(zeros): protein(<50\%), ATAC(~97\%), RNA(~90\%) \citep{li2021chromatin}

	%### TODO
	%##  check the 6 dataset of future number of several 


	\item \textbf{Increasing number of cells:}  The integration methods should be efficient enough to cope with increasing library size. Nowadays, We can sequence as many as even millions of cells.
	\item \textbf{Different modality types make flexibility difficult:} more and more  protocols are created to simultaneously measure features in the same single cell like open chromatin, gene expression, protein, DNA methylation. And the combinations of modality types pose a challenge to the integration, some methods can only tackle 2 modalities, and some others can only work for specific feature matrix
	\item \textbf{Difficulties for interpreting the latent space:} interepration of each component of the latent space, when associate molucular features with the latent space, we can interpreate each component using features from specific modalities.
	association of features (genes, peaks, proteins) to latent spaces
	\item \textbf{overfitting}: MOFA thesis
	\item \textbf{Scalability}: MOFA thesis
	\item \textbf{Discriminating biological vs technical sources of variation}: MOFA thesis
	\item \textbf{Assay noise}
	\item \textbf{Principled validation of the model outputs} evaluation
\end{itemize}
...
\begin{itemize}
	\item \textbf{Complex and multi-branching trajectories not working for these approaches}
	\item \textbf{Most approaches use 2D or 3D to fit trajectories} High dimensional information is not available for these methods
	\item \textbf{Graph-based methods only use the information of nodes} Methods of use Minimum spanning Tree, ElPigraph or diffusion process usually only consider the cell position information. No methods has consider high order information of a graph, like edges and  triangles.
...
Single-cell simultaneous modalities profiling is a nascent area, offering insights into biological processes and cellular heterogeneity. However, the complexity of different modalities' features and the growing library size present challenges, necessitating tools for integration and trajectory inference. In this thesis, we investigate these aspects with two main goals:
 ...
 For single cell multimodal integration:
\begin{itemize}
	\item 
...
\begin{itemize}
	\item 
	The development of a new computational method should involve the ability to infer a trajectory tree effectively capturing cell differentiation events. Additionally, the method should adeptly handle complex tree structure datasets, delving deeper into the graph structure by considering not only the relations between nodes and edges but also the edges and triangles within a graph. Furthermore, the method should be applicable to single-cell multimodal data. To evaluate its efficacy, we will generate high-dimensional simulated data with varying tree structures, ranging from small to large, to assess the model's capability to capture intricate tree structures. Benchmarking will be performed using the dynverse Trajectory Inference evaluation framework, comparing our method with top-ranked methods.

	\item 
	The evaluation of the methods should encompass topological similarity between the inferred trajectory and the ground truth. Additionally, it should assess the accuracy of cell branches belonging and scrutinize the cell order within each branch
