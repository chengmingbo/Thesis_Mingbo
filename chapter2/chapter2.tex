...
We start from a count matrix where the mRNA sequence reads obtained are aligned to genes and cells of origin through an alignment pipeline e.g. 10$\times$ cellranger pipeline~\citep{zheng2017massively}. The pipeline utilizes cell barcodes and unique molecular identifiers~(UMIs) in conjunction with a reference genome. We eventually obtain a count matrix representing genes by cells.
\begin{description}
...
\begin{figure}[!ht]
	...
	\textbf{Step 4:} Downstream analysis including differential accessibility, trajectory inference, peak-to-gene links analysis, TF activity and TF-binding site enrichment analysis.
...
\begin{description}
	...
	\item[Normalization]
	For scATAC data, it is typical to employ the term-frequency inverse-document-frequency~(TF-IDF) method for normalization. This method transforms a cell-to-feature
...
The feature size of surface protein is much smaller~(around 100) compared to other modality types~(\fref{fig:modalities_differences}A). It is often profiled together with scRNA-seq e.g., CITE-seq~\citep{citeseq2017} simultaneously profiles scRNA and surface protein.
