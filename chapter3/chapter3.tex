...
\begin{algorithm}
    ...
    \While{$i \leq N$}
    {
        ...
        \While{\True}{
            $\text{NextNodes} \gets \{\mathcal{V} | \mathcal{V}\in \textbf{Neighbor}(\mathcal{E}^{knn}, \text{node}) \text{ and } U[\mathcal{V}]>\text{node}  \}$\\
            \IF{$\text{NextNodes} == \emptyset$}{
                \Break
            }
        ...
        \While{$k < \text{Len}$}{
            ...
            \If{$(\text{node}_a, \text{node}_b) \in \mathcal{E}^{t}$}{
