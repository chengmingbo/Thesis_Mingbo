...
%    %\begin{Ttabular}{cccccc}
%    \toprule
%      & DIABLO & Liger & MOFA & Schema & scAI & Seurat WNN  \\
%      \midrule
%     problem & PLS-DA & Matrix Factorization & Matrix Factorization & Metrics learning & Matrix Factorization & WNN\\
%     algorithm  & monotonically convergent & coordinate block descent & stochastic variational Bayes & quadratic programming & multiplicative update algorithm & WNN\\
%     time complexity & xx & xx & xx & xx & xx & xx  \\
%     modality types & universal & shared-features & universal & universal & scRNA, scATAC & universal\\
%     \#modalities & any & 2 & any & any & 2 & any \\
%     \#parameters & 2 & 2 & 10+ & 2 & 3 & 2\\
%     inmerpretability yes & yes & yes & no & yes & no\\
%    \bottomrule
%  \end{tabular}
%    %\end{Ttabular}
%  \vspace{0.1cm}
...
\begin{description}
  ...
  \label{eqn:sGCCA}
  \end{equation}
  where $a_h^{(q)}$ is the variable coefficient vector on dimension $h$ associated to the residual matrix $X_q^{(h)}$ of the data matrix $X^{(q)}$, and C=$\{c_{i,j}\}_{i,j}$ is a $Q\times Q$ design matrix to specify whether datasets should be connected. Elements in $C$ can be set to zeros when datasets are not connected and ones where datasets are fully connected. $\lambda(q)$ is a non-negative parameter that controls the amount of shrinkage and thus the number of non-zero coefficients in $a_h^{(q)}$. DIABLO extended the framework to a classification or supervised framework, it substituted one omics dataset $X^{(q)}$ in \eref{eqn:sGCCA} with a dummy indicator matrix $Y\ (N\times G)$ to indicate the class membership of each sample, where $G$ is the number of phenotype groups. DIABLO uses Lagrange multipliers for the optimization problem with constraints. It next obtains a monotonically convergent algorithm to iteratively update $a_h^{i}$ until convergence. DIABLO has two major parameters, apart from the aforementioned $\lambda(q)$: the other is the number of components to retain. 
...
%$\{\sum_{j=2}^{r} \gamma_j Corr(\rho^{*}(w), \rho_j)\}$
%subjec to
%$(Corr(\rho^{*}(w), \rho_j) \geq s$
...
\end{figure}
  ...
  \end{equation}
  where $\textbf{w}_m(i)$ is the weight of cell $i$ in modality $m$ and $\theta(i,j)$ denotes the affinity between cell $i$ and cell $j$ in modality $m$~(\fref{fig:Alg_WNN}). WNN has two major free parameters: the number of neighbors and the scaling factor of the neighborhood kernel.
