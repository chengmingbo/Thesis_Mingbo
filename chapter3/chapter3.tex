...
\begin{algorithm}
    ...
    $CUTs, BINs = [], []$\\
    \For{$i\gets1$ \KwTo $m$ \KwBy $1$}{
        $n_{bin} \gets \lfloor min_{bin} \times \frac{\max({U_i}) - \min({U_i})}{(max_t - min)}\rfloor$\\
    ...
    $invDIC \gets \textbf{invert}(DIC)$ \Comment{invert dictionary: key,val to val,[key]}
    $bps \gets \textbf{order}(\textbf{values}(DIC), ascend=True)$ \Comment{ordered branching points ids}
    $Tree = \emptyset$\\
    \While{$bp \textbf{ in } bps$}{
        $Tree \gets \textbf{MergeBranch}(invDIC, bp)$\Comment{create tree with branching points}
    }
    \Return $Tree$
