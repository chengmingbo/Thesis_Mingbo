...
Although the CCA can calculate as many as $\min(p, q)$, we can not make use all of them. There are two ways to select the top components. One way is to check the correlation of the components between two correlation variables. A second way is to perform a correlation statistical test to exclude unrelated CC pairs. 
