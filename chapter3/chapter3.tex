...
\begin{figure}[!ht]
	...
	\vspace{0.1cm}
	\caption[CCA schematic showing the linear transformation.]{\textbf{CCA schematics showing the linear transformation..} Data is from \texttt{PBMC-Multiome}. \textbf{A)} Same cell $a$ and $b$ in scRNA and scATAC space presents different coordinates. \textbf{B)} After the linear transformation, cell $a$ and $b$ are project to the same space with the angle minimized. Our method add up the the two vectors into for a cell to get a unified coordinate in the CCA space.}
...
\begin{figure}[!ht]
	...
	\vspace{0.1cm}
	\caption[MOJITOO workflow for multimodal data integration.]{\textbf{MOJITOO workflow for multimodal data integration.}}
