...
\begin{figure}[!h]
    ...
    \vspace{0.1cm}
    \caption[Illustration of task of single cell multimodal integration.]{\textbf{Illustration of task of single cell multimodal integration.} The task of single-cell multimodal integration, exemplified here, involves inputting three or more matrices: $\mathbf{X}_a \in \mathbb{R}^{s \times n}$, $\mathbf{X}_b \in \mathbb{R}^{r \times n}$, and $\mathbf{X}_c \in \mathbb{R}^{t \times n}$. The objective is to find a latent space $\mathbf{Z} \in \mathbb{R}^{p \times n}$ that extracts as much important information from the inputs as possible. }
...
\begin{table}[!ht]
	...
	\begin{tabular}{llllllll}
		\toprule
		Name & algorithm & modality types & \#modalities & \#parameters & interpretability & Reference \\
		\midrule
        DIABLO & PLS-DA & universal & any & 2 & yes & \cite{singh2019diablo}\\
        Liger & NMF & shared-features& 2 & 2 & yes & \cite{kriebel2021nonnegative} \\
        MOFA   &  Matrix Factorization & universal & any & 10+ & yes &  \cite{argelaguet2020mofa+} \\
        Schema & Metrics learning  & universal & any & 2 & no & \cite{singh2021schema} \\
        scAI & Matrix Factorization & scRNA,scATAC & 2 & 3 & yes & \cite{jin2020scai}\\
        Seurat WNN   & WNN & universal & any & 2 & no & \cite{hao2021seurat4} \\
		\bottomrule
	\end{tabular}
	\vspace{0.1cm}
...
\begin{description}
  ...
  \label{eqn:sGCCA}
  \end{equation}
  where $a_h^{(q)}$ is the variable coefficient vector on dimension $h$ associated to the residual matrix $X_q^{(h)}$ of the data matrix $X^{(q)}$, and C=$\{c_{i,j}\}_{i,j}$ is a $Q\times Q$ design matrix to specify whether datasets should be connected. Elements in $C$ can be set to zeros when datasets are not connected and ones where datasets are fully connected. $\lambda(q)$ is a non-negative parameter that controls the amount of shrinkage and thus the number of non-zero coefficients in $a_h^{(q)}$. DIABLO extended the framework to a classification or supervised framework, it substituted one omics dataset $X^{(q)}$ in \eref{eqn:sGCCA} with a dummy indicator matrix $Y\ (N\times G)$ to indicate the class membership of each sample, where $G$ is the number of phenotype groups. DIABLO has two major parameters, apart from the aforementioned $\lambda(q)$: the other is the number of components to retain. 
