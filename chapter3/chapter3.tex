...
\begin{algorithm}
    ...
    $\mathcal{V}_{low}^{knn} \gets \textbf{LowestPseudotime}(\text{pct})$ \Comment{pct is percentage parameter}
    ...
    \While{$i \leq N$}
    {
        ...
        \While{$k < \textup{Len}$}{
            ...
            \If{$(\textup{node}_a, \textup{node}_b) \in \mathcal{E}^{t}$}{
                ...
                $L_p[i] \gets \textbf{Append}(L_p[i], \text{NodeList})$\\
            }\Else{
                $L_p[i] \gets \textbf{Append}(L_p[i], \text{node}_a)$\\
            }
        }
...
\begin{algorithm}
    ...
    % g is trajectory paths group, V is the cumulative coordinates, U is the edges pseudotime
    %
    %1. shortest trajectory to obtain number of bins
    %2. get bins of each trajectory group
    %3. pairwise groups to get branching point
    %4. get a cell fate tree
    $min_t, max_t, max_{range} \gets (\infty, -\infty, -\infty)$\\
    \For{$i\gets1$ \KwTo $m$ \KwBy $1$}{
        $min_t \gets \min(U_i) \textbf{if} \min(U_i)<min_t \textbf{else} min_t$\\
        $max_t \gets \max(U_i) \textbf{if} \min(U_i)>max_t \textbf{else} max_t$\\
        $max_{range} \gets (\max({U_i})-\min({U_i})) \textbf{if} (\max({U_i})-\min({U_i}))>max_{range} \textbf{else} max_{range}$\\
    }
    $CUTs, BINs = [], []$\\
    \For{$i\gets1$ \KwTo $m$ \KwBy $1$}{
        $n_{bin} \gets \lfloor min_{bin} \times (\max({U_i}) - \min({U_i})) / (max_t - min))\rfloor$\\
        $CUTs[i] \gets \textbf{linspace}(\max({U_i}), \min({U_i}), n_{bin}+2)[2:-1]$\\
        $BINs[i] =[U_i^{1}, U_i^{2}, \cdots U_i^{n_{bin}}]$\\
        $V_i =[V_i^{1}, V_i^{2}, \cdots V_i^{n_{bin}}]$ \Comment{split the $V_{i}$ to be $n_{bin}$}
    }
    $DIC = \{\}$\\
    \For{$i\gets1$ \KwTo $m$ \KwBy $1$}{
        \For{$j\gets i+1$ \KwTo $m$ \KwBy $1$ \& j \neq i}{
            $\text{Len}= min(\textbf{length}({V_i), \textbf{length}({V_j))$\\
            \For{$k\gets Len$ \KwTo $1$ \KwBy $-1$}{
                ...
                \If{$\mathrm{d}(i,j,k) < cut_{threshold}$}{
                    $DIC(i,j) \gets k$\\
                    $\textbf{break}$\\ 
                }
            }
    ...
    $inv_DIC = \textbf{invert}{DIC}$ \Comment{invert dictionary: key,val to val,{key}}
    $bps \gets \textbf{order}(\textbf{values}(DIC), ascend=True)$ \Comment{ordered branching points ids}
    $Tree = \emptyset$\\
    \While{$bp \textbf{ in } bps$}{
        $Tree \gets \textbf{MergeBranch}(inv_DIC, bp)$\Comment{create tree with branching points}
    }
    \Return $Tree$
