...
Another aspect is that should we keep all components from CCA. Although the CCA can calculate as many as $\min(p, q)$, we can not make use all of them since the last several components are almost irrelevant, see fig \fref{fig:CC_corrlations_last}. There are two ways to select the top components. One way is to check the correlation of the components between two correlation variables. A second way is to perform a correlation statistical test to exclude unrelated CC pairs. 
